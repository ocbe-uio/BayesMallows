...
test_that('MH-aug ranking works', {
	...
	test_1 = metropolis_hastings_aug_ranking(current_ranking = R_curr, partial_ranking= R_obs, alpha = alpha, rho = rho, n_items = n_items, metric = metric)
	expect_equal(test_1, as.matrix(c(1, 2, 3, 5, 6, 4)))
	expect_equal(get_rank_distance(rho, test_1, metric= "ulam"), 1)
	R_curr = rho
	R_obs = c(1,2,3,NA,NA,NA)
	test_2 = metropolis_hastings_aug_ranking(current_ranking = R_curr, partial_ranking = R_obs, alpha = alpha, rho = rho, n_items = n_items, metric = metric)
	...
	test_3 = metropolis_hastings_aug_ranking(current_ranking = R_curr, partial_ranking = R_obs, alpha = alpha, rho = rho, n_items = n_items, metric = metric)
