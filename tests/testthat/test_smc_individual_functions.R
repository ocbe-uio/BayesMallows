...
test_that("smc_metropolis_hastings_rho() works as expected", {
	...
	# expect_equal(test_3, c(1, 3, 4, 2, 5, 6)) # FIXME: failing test
	# expect_equal(dist_3, 1) # FIXME: failing test
...
test_that("smc_leap_and_shift_probs() works as expected", {
	...
	# expect_equal(test_2$rho_prime, c(3, 1, 2, 4, 5, 6)) # FIXME:fails
	# expect_equivalent(test_2$forwards_prob, 0.05555556, tol=1e-6) # FIXME: fails
	# expect_equivalent(test_2$backwards_prob, 0.03333333, tol=1e-6) # FIXME: fails
