...
test_that("MH-aug ranking works", {
  ...
  test_2 <- metropolis_hastings_aug_ranking(
    current_ranking = R_curr, partial_ranking = rankings, alpha = alpha,
...
test_that("correction_kernel works", {
  ...
  rankings <- c(1, 2, 3, NA, NA, NA)
  set.seed(879)
  test_4 <- correction_kernel(rankings, R_curr, n_items)
  ...
  set.seed(706)
  test_5 <- correction_kernel(rankings, R_curr, n_items)
  ...
  set.seed(731)
  test_6 <- correction_kernel(rankings, R_curr, n_items)
