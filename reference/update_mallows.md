...
algorithm in [`compute_mallows()`](compute_mallows.md) using the
sequential Monte Carlo algorithm described in Stein (2023) .
...
- model:
  ...
  [`compute_mallows()`](compute_mallows.md), an object of class
  "SMCMallows" returned from this function, or an object of class
  "BayesMallowsPriorSamples" returned from
  [`sample_prior()`](sample_prior.md).

- new_data:

  An object of class "BayesMallowsData" returned from
  [`setup_rank_data()`](setup_rank_data.md). The object should contain
  the new data being provided.
...
- priors:
  ...
  [`set_priors()`](set_priors.md). Defaults to the priors used in
  `model`.
...
- pfun_estimate:
  ...
  [`get_cardinalities()`](get_cardinalities.md). Only used by the
  specialization for objects of type "BayesMallowsPriorSamples".
