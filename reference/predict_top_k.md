...
top-\\k\\ for each assessor. This is useful when the data take the form
...
A dataframe with columns `assessor`, `item`, and `prob`, where each row
states the probability that the given assessor rates the given item
among top-\\k\\.
