...
//'
// [[Rcpp::export]]
Rcpp::List run_mcmc(arma::mat rankings, arma::vec weights, int nmc,
  ...
  update_dist_mat(dist_mat, rankings, rho_old, metric, weights);
  ...
  for(int t = 1; t < nmc; ++t){
    ...
    for(int i = 0; i < n_clusters; ++i){
      update_rho(rho, rho_acceptance, rho_old, rho_index, i,
                 ...
                 metric, n_items, t, element_indices, weights);
    ...
    if(t % alpha_jump == 0) {
      ...
      for(int i = 0; i < n_clusters; ++i){
        alpha(i, alpha_index) = update_alpha(alpha_acceptance, alpha_old(i),
              clustering ? rankings.submat(element_indices, arma::find(current_cluster_assignment == i)) : rankings,
              weights, i, rho_old.col(i), alpha_prop_sd, metric, lambda, cardinalities, logz_estimate, alpha_max);
      }
      // Update alpha_old
