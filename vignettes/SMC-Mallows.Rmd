...
model2 <- update_mallows(mod_init, 
...
By compare the non-missing ranks, we can check if they are consistent or not:
...
model3 <- update_mallows(
  mod_init, new_data = setup_rank_data(rankings = data_batch3))
```
...
Now assume we get a batch of new users, without missing ranks. These can be treated just as the other ones.


```r
data_batch4 <- sushi_rankings[500:600, ]
model4 <- update_mallows(
  model3, new_data = setup_rank_data(rankings = data_batch4))
```
