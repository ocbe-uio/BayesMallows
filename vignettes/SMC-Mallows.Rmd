...
The function `smc_mallows_new_users()` shows the simplest version of the SMC algorithm in action. The variable `rankings` in this instance is a matrix whose of the same format as used in `compute_mallows()`, i.e., with one column for each item and one row for each user. 

```{r}
data_batch1 <- sushi_rankings[1:300, ]
model1 <- compute_mallows(rankings = data_batch1)
...
model2 <- update_mallows(model1, new_rankings = data_batch1, n_particles = 1000)
```


```{r}
data_batch3 <- sushi_rankings[601:900, ]
...
data_partial <- sushi_rankings[sample(nrow(sushi_rankings), 100), ]
...
#### New users with partial rankings

If a new user arrives, again with partial rankings, we can use the `smc_mallows_update()` function as we did for complete data.

Assume a new batch of 100 sushi preferences arrives, again with top-5 rankings:
...
```{r}
mod_updated <- smc_mallows_update(
  model = partial_mod1,
  rankings = data_partial
)
```

Again we can plot the posterior density of $\alpha$, which now is based on both the new data and on the old data. 

```{r}
plot(mod_updated)
```

In this example, the data is rather noisy because of the high amount of missingness and the low number of observations. On [this link](https://github.com/ocbe-uio/BayesMallows/blob/smc-issue-306/tests/testthat/test-smc_updated_new_item_rank.R) you can find long-running tests which confirm the correctness of the implementation, by comparing the results on the full sushi data with the results obtained by analyzing the whole data jointly using the Metropolis-Hastings in the `compute_mallows()` function.
