...
We can now gradually update the models, using `smc_mallows_update()`, and keep track of the consensus probabilities, to see how they get updated.
...
for(i in seq(from = 2, to = length(mod_list))) {
  mod_list[[i]] <- smc_mallows_update(
    model = mod_list[[i - 1]], 
    rankings = potato_partial[,, i, drop = FALSE], 
    verbose = TRUE)
}
```
...
```{r}
consensus_over_time <- do.call(rbind, lapply(seq_along(mod_list), function(i) {
  cc <- compute_consensus(mod_list[[i]])
  cbind(step = i, cc)
}))
```
...
```{r}
subset(consensus_over_time, item == "Item 6")
```
