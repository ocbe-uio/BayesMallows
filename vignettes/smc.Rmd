...
| `compute_consensus_rho`| Computes the CP estimate or MAP estimate of the latent ranks. |
...
BayesMallows:::plot_rho_heatplot(output = smc_test$rho_samples[,,Time+1], 
...
BayesMallows:::compute_posterior_intervals_rho(output = test_sample_rho, 
                                               nmc = N, burnin = 0, 
                                               verbose = FALSE)
```

We can also rank the sushi items according to their cumulative probability (CP) consensus and their maximum posterior rankings (MAP). These are calculated by calling the function `compute_consensus_rho`. We demonstrate with the CP consensus:

```{r consensus_ranking_estimates, message=FALSE, warning=FALSE}
BayesMallows:::compute_rho_consensus(output = test_sample_rho, nmc = N, 
...
BayesMallows:::plot_alpha_posterior(output = test_sample_alpha, nmc = N, 
                                    burnin = 0, verbose = TRUE)

BayesMallows:::compute_posterior_intervals_alpha(output = test_sample_alpha, 
                                                 nmc = N, burnin = 0, 
                                                 verbose = FALSE)
```
...
rho_0 = c(4,5,2,6,8,3,9,1,7,10)
BayesMallows:::plot_rho_heatplot(output = partial_test_sample_rho, nmc = N, 
                                 burnin = 0, n_items = n_items, rho_0 = rho_0)

BayesMallows:::plot_alpha_posterior(output = partial_test_sample_alpha, nmc = N, 
                                    burnin = 0, verbose = TRUE)
...
BayesMallows:::plot_rho_heatplot(output = updated_partial_test_sample_rho, nmc = N, 
                                 burnin = 0, n_items = n_items, rho_0 = rho_0)

BayesMallows:::plot_alpha_posterior(output = updated_partial_test_sample_alpha, nmc = N, 
                                    burnin = 0, verbose = TRUE)
