...
The element `n_clusters` is correctly equal to 2. In addition, note that `rho` has size $20 \times 3000 \times 2$, and that `rho_acceptance`, `alpha`, and `alpha_acceptance` have size $3000 \times 2$.

**Note:** The package author has a job to do in terms of defining the dimensions of the various parameter objects, cf. `dim(model_fit)$cluster_probs` vs. `dim(model_fit)$cluster_indicator`. This also has performance implications, due to [C++'s row-major ordering](https://en.wikipedia.org/wiki/Row-_and_column-major_order#Programming_languages_and_libraries).
...
potato_true_ranking
plot(model_fit, burnin = burnin, type = "rho", items = which(potato_true_ranking <= 3))
```

We do the same with the three lightest potatoes.

```{r, fig.width=6, fig.height=4}
plot(model_fit, burnin = burnin, type = "rho", items = which(potato_true_ranking >= 18))
```
