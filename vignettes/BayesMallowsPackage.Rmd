...
We can also find the posterior probability for each beach of being ranked top-$k$, both in $\rho$ and among the assessors. In order to find this for the assessors, we need to set `save_augmented_data = TRUE`.
...
We can do a top-$k$ plot with `plot_top_k`. By default, `k = 3`.
```{r, eval=FALSE}
plot_top_k(model_fit, burnin = 2000)
```

```{r, echo=FALSE, out.width="75%"}
knitr::include_graphics("./vignette-data/top_k_beaches.png")
```
