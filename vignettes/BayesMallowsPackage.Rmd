...
```{r, eval=FALSE}
compute_map_consensus(model_fit, burnin = 1000)
```


```{r, results='asis', echo=FALSE}
knitr::kable(compute_map_consensus(model_fit, burnin = 1000), digits = 2)
```
...
Choosing 5 clusters seems to be an ok solution. We can then find the cluster assignment probabilities for each assessor.

```{r, eval=FALSE}
plot(models[[5]], burnin = 1000, type = "cluster_assignment")
```
...
```{r, eval=FALSE}
cluster_assignment <- assign_cluster(models[[7]], 
                                     burnin = 1000, soft = FALSE)
```
