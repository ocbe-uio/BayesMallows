...
We also load some other packages which are useful.
...
When the name of the items have been specified in the column names of `rankings`, we can also provide these to the `items` argument. The line below plots potatoes 16 through 20.
...
```{r}
model_fit <- compute_mallows(potato_top, aug_diag_thinning = 10, nmc = 10000)
```
...
If we want to take a closer look at some of the curves, we can specify this with the `assessors` argument. Let's look at assessor 3, which seems to have a very high acceptance rate.

```{r, fig.show='hold'}
assess_convergence(model_fit, type = "augmentation", assessors = 3)
```
...
```{r}
model_fit <- compute_mallows(rankings = initial_ranking, preferences = pair_comp_tc)
```
...
```{r}
knitr::kable(filter(cp_consensus_df, cluster == "Cluster 1"), digits = 2)
```
...
We can use the function `compute_within_cluster_distance` to find the posterior distribution of the within-cluster footrule distance to the cluster centers. We need to supply the original `sushi_rankings` matrix, as well as the burn-in.
...
To get other quantiles, use the `probs` argument to `compute_within_cluster_distance`. To speed up the computation, use the `thinning` argument (with caution!).

We can also compute an elbow plot directly, using `plot_elbow`. Before doing that, we should look at its arguments:
```{r}
formals(plot_elbow)
```

`plot_elbow` requires the model fits for different number of clusters to be provided as the first arguments. The remaining arguments must be named. We try this for the case of one and two clusters:
