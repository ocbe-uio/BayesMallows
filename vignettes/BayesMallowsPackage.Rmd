...
```{r, fig.width=6}
assess_convergence(model_fit, type = "rho", items = c("P2", "P11", "P14", "P18", "P19", "P20"))
```
...
```{r}
model_fit <- compute_mallows(rankings = initial_ranking, preferences = pair_comp_tc, save_augmented_data = TRUE)
```
...
Below, we study items 2 and 3 for assessors 1. As ecpected, item 3 fluctuates wildly, since it is not constrained. Item 2 jumps between being ranked 2 and 3.

```{r, fig.width=6}
assess_convergence(model_fit, type = "augmentation", assessors = 1, items = 2:3)
```
...
Let us add column names to the initial ranking. This will make the plots more informative.

```{r}
colnames(beach_init_rank) <- paste("Beach", seq(from = 1, to = ncol(beach_init_rank), by = 1))
```
...
#### Convergence of $\tilde{R}$

We can set `type = "augmentation"` and choose `assessors` and `items`.

##### Assessor 1
Let us start by looking at assessor 1:
```{r, fig.width=6}
assess_convergence(test_run, type = "augmentation", assessors = 1, items = 1:15)
```

All rankings of assessor 1 are fixed. We can see why by looking at the implied linear ordering of assessor 1's preferences:

```{r}
test_run$linear_ordering[[1]]
length(test_run$linear_ordering[[1]])
```

All 15 items are constrained, so the ranking for assessor 1 is fixed.

##### Assessor 2
For assessor 2, many of the beaches are fluctuating.
```{r, fig.width=6}
assess_convergence(test_run, type = "augmentation", assessors = 2, items = 1:15)
```

The fluctuation makes sense, because assessor 2 has stated preferences for 12 beaches out of 15.
```{r}
test_run$linear_ordering[[2]]
length(test_run$linear_ordering[[2]])
```

We can confirm the consistency of the augmented rankings by studying a selected set. Let us look at beaches 2, 7, and 8, i.e., the bottom-3:

```{r, fig.width=6}
assess_convergence(test_run, type = "augmentation", assessors = 2, 
                   items = c(2, 7, 8))
```

Indeed, the internal ordering of beaches 2, 7, and 8 is always consistent with the linear ordering.
...
```{r, eval=FALSE}
plot_top_k(model_fit, burnin = 1000)
```
