...
#### Latent Ranks $\rho$
Obtaining posterior samples from $\rho$ is in general harder than for $\alpha$. Some items tend to be very sticky. We start by plotting the `model_fit` object from above. We now have to tell `plot.BayesMallows` that we want a plot of `parameter = "rho"` and all the items. This gives us posterior the posterior density of all the items.
...
We only need the within-cluster distances to compute the elbow plot. Thus, we can basically "thin out" all the other parameters, since we do not need them. This reduces the amount of memory used.
...
Let us choose 5 clusters. We then compute this model, now without the extreme thinning, and also saving cluster assignments.
