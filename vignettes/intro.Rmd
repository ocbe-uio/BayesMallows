...
When the name of the items have been specified in to column names of `R`, we can also provide these to the `items` argument:
...
#### Jumping over Alphas
We try again with more samples. To avoid updating $\alpha$ in each step, we set `alpha_jump = 100`. This implies that $\alpha$ is updated and saved only every 10th iteration on $\rho$. Note that this is not thinning, since $\alpha$ is never sampled. Before computing the posterior distribution with this parameter, we do a new convergence assessment:
