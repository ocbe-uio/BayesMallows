...
Saving the augmented ranks through every MCMC step would yield an array of size `nmc x n_items x n_assessors`, which quickly becomes prohibitively large. Thus, rather than saving this, we save an array of size `nmc x n_assessors / aug_diag_thinning`, where `aug_diag_thinning` is an argument to `compute_mallows`. We save the average acceptance rates for each iteration interval of size `aug_diag_thinning`. The default value is 100.
...
If the ranks are missing completely at random, we cannot remove the unranked items as we did for top-k rankings above. Let us assume that 10 % of the data in `potato_visual` have disappeared due to a disk failure. We generate this data now:
