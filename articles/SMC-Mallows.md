...
This vignette describes sequential Monte Carlo (SMC) algorithms to
...
treatment of the implemented methodology, see Stein
([2023](#ref-steinSequentialInferenceMallows2023)) which is available
[here](https://eprints.lancs.ac.uk/id/eprint/195759/).

## New users with complete rankings

We use the `sushi_rankings` dataset to illustrate the methodology
([Kamishima 2003](#ref-kamishima2003nantonac)). This dataset contains
5000 complete rankings for 10 sushi dishes.
...
The SMC methodology is designed for the case where date arrive in
...
We estimate a model on these data using
[`compute_mallows()`](../reference/compute_mallows.md), which runs a
...
![Trace plot for SMC model.](convergence_smc_full-1.png)
...
Having saved this model, assume we receive another batch of preferences
at a later timepoint, with an additional 300 rankings.

``` r
data_batch2 <- sushi_rankings[301:600, ]
```

We can now update the initial model, without rerunning the full
Metropolis-Hastings algorithm, by calling
[`update_mallows()`](../reference/update_mallows.md). This function uses
the sequential Monte Carlo algorithm of Stein
([2023](#ref-steinSequentialInferenceMallows2023)), and extracts a
thinned sample of size `n_particles` from `model1` as initial values.
...
All the posterior summary methods can be used for `model2`. For example,
...
![Posterior distribution of scale parameter for model
2.](smc_complete_model2_alpha-1.png)
...
![Posterior distribution of selected latent rankings for model
2.](smc_complete_model2-1.png)
...
We can again plot posterior quantities, and the plots reveal that as
...
![Posterior distribution of selected latent rankings for model
4.](smc_complete_model4_rho-1.png)

Posterior distribution of selected latent rankings for model 4.

Below is a comparison of the posterior intervals of the dispersion
parameter for each model. Note how the intervals get increasingly
narrower as more data is added.
...
As an assurance that the implementation is correct, we can compare the
final model to what we get by running `compute_mallows` on the complete
dataset:
...
We can compare the posteriors for \\\alpha\\ of the two models. Note
that although both are rather wiggly, they agree very well about
location and scale.
...
![Posterior distribution of scale parameter for Metropolis-Hastings run
on the complete data.](smc_complete_mod_bmm-1.png)
...
![Posterior distribution of scale parameter for model
4.](smc_complete_model4_alpha-1.png)
...
The functionality extends directly to partial ranks, including both
top-\\k\\ rankings and rankings missing at random. Pairwise preferences
are also supported, although not demonstrated here.

For this demonstration we shall assume that we can only observe the
top-5 ranked items from each user in the `sushi_rankings` dataset.
...
Again, assume we start out with a batch of data, this time with 100
rankings:
...
[`compute_mallows()`](../reference/compute_mallows.md). Since there are
`NA`s in the data,
[`compute_mallows()`](../reference/compute_mallows.md) will run
imputation over the missing ranks.
...
![Trace plot for SMC model.](convergence_smc_partial-1.png)
...
![Posterior distribution of scale parameter after initial
run.](smc_init_posterior_partial-1.png)
...
We now update the initial model, using SMC. By default, a uniform
distribution is used to propose new values of augmented ranks. The
pseudo-likelihood proposal developed in Stein
([2023](#ref-steinSequentialInferenceMallows2023)) can be used instead,
by setting `aug_method = "pseudo"` in the call to
[`set_compute_options()`](../reference/set_compute_options.md), and we
do this here.
...
![Posterior distribution of scale parameter after updating the model
based on new rankings.](smc_updated_posterior_partial-1.png)

Posterior distribution of scale parameter after updating the model based
on new rankings.

When even more data arrives, we can update the model again. For example,
assume we now get a set of complete rankings, with no missingness:
...
![Posterior distribution of scale parameter after updating the model
based on new rankings.](smc_second_updated_posterior_partial-1.png)
...
Another setting supported is when existing users update their partial
...
subsequently provide these rankings.

The main methodological issue in this case, is that the augmented
...
described in Chapter 6 of Stein
([2023](#ref-steinSequentialInferenceMallows2023)). We provide an
example again with the sushi data.

We assume that the initial batch of data contains top-3 rankings
provided by the first 100 users.
...
To keep track of existing users updating their preferences, we also need
a user ID in this case, which is required to be a number vector.
...
We fit the standard Metropolis-Hastings algorithm to these data,
yielding a starting point.
...
![Trace plot for initial run on sushi batch
1.](sushi_updated_batch1_burnin-1.png)
...
Next, assume we receive top-5 rankings from the same users. We now
update the model using SMC.
...
![Posterior after sushi batch 1.](sushi_updated_batch1_posterior-1.png)
...
![Posterior after sushi batch 2.](sushi_updated_batch2_posterior-1.png)
...
Before proceeding, it is instructive to study why this situation needs
special care. Below are the augmented rankings for user 1 in particle 1:
...
By comparing the non-missing ranks, we can check if they are consistent
or not:
...
The provided data are not consistent with the augmented rankings in this
case. This means that the augmented rankings for user 1 in particle 1
need to be corrected by the algorithm.

Luckily, this happens automatically in our implementation, so we can
...
![Posterior after sushi batch 3.](sushi_updated_batch3_posterior-1.png)

Posterior after sushi batch 3.

Now assume we get a batch of new users, without missing ranks. These can
be treated just as the other ones, but we need new user IDs.
...
![Posterior after sushi batch 4.](sushi_updated_batch4_posterior-1.png)

Posterior after sushi batch 4.

We can confirm that the implementation is sensible by giving the
complete data to `compute_mallows`:
...
![Trace plot for MCMC run on sushi data.](sushi_updated_burnin-1.png)
...
![Posterior for MCMC on sushi data.](sushi_updated_bmm_posterior-1.png)

Posterior for MCMC on sushi data.

## References

Kamishima, Toshihiro. 2003. “Nantonac Collaborative Filtering:
Recommendation Based on Order Responses.” In *Proceedings of the Ninth
