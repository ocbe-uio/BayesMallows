get_mallows_loglik <- function(alpha, rho, n_items, rankings, metric){
  ...
  if (is.null(num_rankings)){
    sum_distance = sum_distance + BayesMallows:::get_rank_distance(rho, rankings, metric = metric)
  }else{
    for (jj in 1:num_rankings){
      sum_distance = sum_distance + BayesMallows:::get_rank_distance(rho, rankings[jj,], metric = metric)
    }
  }
