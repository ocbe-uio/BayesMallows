...
smc_mallows_new_item_rank = function(n_items, R_obs, metric, leap_size, N, Time, logz_estimate, mcmc_kernel_app,
  ...
  for (ii in 1:N){
    ...
    for (jj in 1:num_ranks){
      ...
      if(aug_method == "random"){
        ...
        if(length(remaining_set == 1)){partial_ranking[is.na(partial_ranking)] = remaining_set}
        else {partial_ranking[is.na(partial_ranking)] <- sample(remaining_set, size=length(remaining_set), replace=F)}
        ...
        proposal = calculate_forward_probability(item_ordering = item_ordering, partial_ranking = R_obs[jj,,1],
  ...
  for (ii in 1:N){ 
    ...
    log_z_alpha = BayesMallows:::get_partition_function(n_items = n_items, alpha =  alpha_samples[ii,1], 
                                                        logz_estimate = logz_estimate, metric = metric)
    log_likelihood = get_mallows_loglik(alpha = alpha_samples[ii,1], rho = rho_samples[ii,,1], 
  ...
  for (ii in 1:N){  
    for (kk in 1: mcmc_kernel_app) {
      rho_samples[ii,,1] = metropolis_hastings_rho(alpha = alpha_samples[ii,1], n_items = n_items, 
                                                   rankings = aug_rankings[,,ii], metric = metric, 
                                                   rho = rho_samples[ii,,1], leap_size = leap_size)
  
      alpha_samples[ii,1] = metropolis_hastings_alpha_update(alpha = alpha_samples[ii,1], n_items = n_items, 
                                                             rankings = aug_rankings[,,ii], metric = metric, 
    ...
    for (jj in 1:num_ranks){ 
      if (aug_method == "random"){
        aug_rankings[jj,,ii] = metropolis_hastings_aug_ranking(alpha = alpha_samples[ii,1],  
                                                                      rho = rho_samples[ii,,1], n_items = n_items, 
                                                                      R_obs = R_obs[jj,,1], 
                                                                      R_curr = aug_rankings[jj,,ii], metric = metric)
        
      }else if((aug_method == "pseudolikelihood") & (metric %in% c("footrule", "spearman")) ){
        aug_rankings[jj,,ii] = metropolis_hastings_aug_ranking_pseudo(alpha = alpha_samples[ii,1],  
                                                                      rho = rho_samples[ii,,1], n_items = n_items, 
                                                                      partial_ranking = R_obs[jj,,1], 
  ...
  for (tt in 1:(Time-1)){
    ...
    for (ii in 1:N){
      ...
      for (jj in 1:num_ranks){
        ...
        if(aug_method == "random"){
          ...
          check_correction  = correction_kernel(R_curr = aug_rankings[jj,,ii], 
          ...
          check_correction = correction_kernel_pseudo(R_curr = aug_rankings[jj,,ii], 
    ...
    for (ii in 1:N){ 
      log_inc_wgt[ii] = get_mallows_loglik(alpha = alpha_samples[ii,tt+1], rho = rho_samples[ii,,tt+1], 
                                           n_items =n_items, rankings = aug_rankings[,,ii], metric = metric) - 
        get_mallows_loglik(alpha = alpha_samples[ii,tt+1], rho = rho_samples[ii,,tt+1],   
...
smc_mallows_new_item_rank_alpha_fixed = function(alpha, n_items, R_obs, metric, leap_size, N, Time, logz_estimate, mcmc_kernel_app,
  ...
  for (ii in 1:N){ 
    ...
    log_z_alpha = BayesMallows:::get_partition_function(n_items = n_items, alpha =  alpha, 
                                                        logz_estimate = logz_estimate, metric = metric)
    log_likelihood = get_mallows_loglik(alpha = alpha, rho = rho_samples[ii,,1], 
                                        n_items = n_items, rankings = aug_rankings[,,ii], metric = metric) 
    log_inc_wgt[ii] = log_likelihood - num_ranks*log_z_alpha - log(total_correction_prob[ii])
  }
  ...
  for (ii in 1:N){
    for (kk in 1: mcmc_kernel_app) {
      rho_samples[ii,,1] = metropolis_hastings_rho(alpha = alpha, n_items = n_items, 
                                                   rankings = aug_rankings[,,ii], metric = metric, 
                                                   rho = rho_samples[ii,,1], leap_size = leap_size)
    ...
    for (jj in 1:num_ranks){ 
      if (aug_method == "random"){
        aug_rankings[jj,,ii] = metropolis_hastings_aug_ranking(alpha = alpha, 
                                                                      rho = rho_samples[ii,,1], n_items = n_items, 
                                                                      R_obs = R_obs[jj,,1], 
                                                                      R_curr = aug_rankings[jj,,ii], metric = metric)
        
      }else if((aug_method == "pseudolikelihood") & (metric %in% c("footrule", "spearman")) ){
        aug_rankings[jj,,ii] = metropolis_hastings_aug_ranking_pseudo(alpha = alpha, 
                                                                      rho = rho_samples[ii,,1], n_items = n_items, 
                                                                      partial_ranking = R_obs[jj,,1], 
                                                                      current_ranking = aug_rankings[jj,,ii], metric = metric)
      }
