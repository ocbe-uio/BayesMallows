...
#'
#' @description Compute either the likelihood or the log-likelihood value of the Mallows mixture model parameters for a dataset of complete rankings.
...
#' @param alpha A vector of \code{n_clusters} non-negative scalar specifying the scale (precision) parameters of the Mallows mixture components.
...
#' @param log A logical; if TRUE, the log-likelihood value is returned. Default is \code{FALSE}.
#'
#' @return The likelihood or the log-likelihood value corresponding to one or more observed complete rankings under the Mallows mixture rank model with distance specified by the \code{metric} argument.
#' @export
#'
#' @example /inst/examples/Lik_DB_MIX_example.R
#'
Lik_DB_MIX <- function(rho,alpha,weights,metric,rankings,obs_freq=NULL,log=FALSE){
