    ...
    for (ii in 1:N) {
      for (jj in (num_obs - num_new_obs + 1):num_obs) {
        ...
        if (aug_method == "random") {
          ...
          if (length(missing_ranks == 1)) {
            partial_ranking[is.na(partial_ranking)] <- missing_ranks
          }
          else {
            partial_ranking[is.na(partial_ranking)] <- sample(missing_ranks, size = length(missing_ranks), replace = F)
          }
