...
compute_mallows <- function(rankings = NULL,
  ...
  if (!is.null(rankings)) {
    if (model$na_action == "fail" && any(is.na(rankings))) {
      stop("rankings matrix contains NA values")
    }

    if (model$na_action == "omit" && any(is.na(rankings))) {
  ...
  if (is.null(preferences) && model$error_model != "none") {
  ...
  } else if (model$error_model != "none") {
  ...
  logz_list <- prepare_partition_function(logz_estimate, model$metric, n_items)
  ...
  if (is.null(cl)) {
    ...
    parallel::clusterExport(
      cl = cl,
      varlist = c(
        "rankings", "obs_freq", "model", "compute_options", "priors", "init",
        "constraints", "logz_list", "verbose"
      ),
      envir = environment()
    )
  ...
  fit <- tidy_mcmc(fits, rankings, model, n_items, compute_options)
