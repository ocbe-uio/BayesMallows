...
metropolis_hastings_aug_ranking_pseudo = function(alpha, rho, n_items, partial_ranking, current_ranking, metric){
  ...
  if(identical(partial_ranking,current_ranking) == TRUE){
    ...
    curr_logpost = get_mallows_loglik(alpha = alpha, rho = rho, n = n_items, rankings = current_ranking, metric = metric)
    prop_logpost = get_mallows_loglik(alpha = alpha, rho = rho, n = n_items, rankings = proposed_augmented_ranking, metric = metric)
